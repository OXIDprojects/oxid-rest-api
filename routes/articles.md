...
{% api-method-description %}
Create a new article. If id parameter OXID is not provided, a value will be created automatically. The article is added to the oxarticles table directly. Add "/object/" to the URL to use the OXID framework for creation instead, which internally uses Article::save\(\) and therefore calls all related hooks etc.
{% endapi-method-description %}
...
{% api-method-description %}
Delete an article from the database via id. To use the OXID framework for deletion, add "/object/" to the URL.
{% endapi-method-description %}
